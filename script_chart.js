document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateData() {
        Promise.all([
            ...
            if (weatherData && airData) {
                ...
                console.log('Air Components:', airData.list[0].components);
                ...
                if (document.getElementById('so2')) {
        ...
        }).catch(error => console.error('Error in data fetching or processing:', error));
    }
    ...
    function toggleSidebar() {
        ...
        toggleButton.style.display = isSidebarVisible ? 'block' : 'none'; // Hide button when sidebar is visible
    }
    ...
    document.addEventListener('click', (event) => {
        ...
        if (!isClickInsideSidebar && !isClickInsideButton && sidebar.style.display === 'block') {
            hideSidebar();
        }
    });
